...
% Table~\ref{tab:notations} shows the basic notation used in this paper. 
...
\begin{figure}[t]
  ...
  \vspace{-2mm}
  \caption{Relative errors of the triangle count, 2-star count, and clustering coefficient when \AlgWSTriVR{} and the one-round local 2-star algorithm in \cite{Imola_USENIX21} with edge clipping are used ($n=107614$ in \Gplus{}, $n=896308$ in \IMDB{}, $c=1$).
  }
  \label{fig:res6_clst}
\end{figure}
...
\begin{figure}[t]
  ...
  \vspace{-2mm}
  \caption{Comparison with the two-round local algorithm in \cite{Imola_USENIX22}.
  ...
  \label{fig:res3_2rounds}
\end{figure}

Figure~\ref{fig:res3_2rounds} shows the results.
...
\begin{figure}[t]
  ...
  \vspace{-2mm}
  \caption{Relative error in the BA graph data
  ($n=107614$, $c=1$).
...
%\begin{figure}[t]
  ...
  \vspace{-2mm}
  \caption{Box plots of counts/estimates in the BA graph data
  ($n=107614$, $c=1$).
  ...
  \label{fig:res7_BA_box}
\end{figure}

Figures~\ref{fig:res7_BA} shows that when $\epsilon=1$, the relative errors of \AlgWSTriVR{} ($m=100$), \AlgWSCyc{} ($m=100$), \AlgWSTriVR{} ($m=200$), and \AlgWSCyc{} ($m=200$) are $1.36$, $0.447$, $0.323$, and $0.0928$, respectively.
...
Figure~\ref{fig:res9_std_err} shows the standard error of the average relative error in Figure~\ref{fig:res1_eps}. 
...
% \section{Proofs of Statements in Section~\ref{sec:triangle}}
\section{Proofs of Statements in Section 5}
\label{sec:triangle_proof}
% Below, we provide proofs of statements in Section~\ref{sec:triangle}.
% Hereinafter, a subscript $r$ in $\E_r$, $\V_r$, or $\cov_r$ represents that $\E$, $\V^+$, or $\cov$ is taken over $r$.

% \subsection{Proof of Theorem~\ref{thm:DP_I}}
% \label{sub:DP_I_proof}
% TBD
...
\label{sub:unbiased_I_proof}
    ...
    \smallskip
    \noindent{\textbf{First Estimate.}}~~Since variables $z_{i,j}$ and $y_{k}$ in~\eqref{eq:est1} are independent, we have
    \begin{equation*}
...
\begin{lemma}\label{lem:sampling_replacement_var}
...
\subsection{Proof of Theorem~\ref{thm:bias_II_ast}}
...
% \subsection{\colorB{Proof of Theorem~\ref{thm:var_II_ast_wo_Lap}}}
% \label{sub:var_II_ast_proof_wo_Lap}
% Similar to the process in the proof of Theorem~\ref{sub:l2-loss_II_proof},
% the variance of $\hf_*^\triangle(G)$ can be written by using (\ref{eq:hf_triangle_II_ast}) as follows:
% \begin{align}
%     & \V_{\sigma,RR}[\hf_*^\triangle(G)] \nonumber\\
%     &= \frac{n^2(n-1)^2}{36t^2}
%     \V_\sigma\left[\sum_{i \in D}
%     \E_{RR}\left[\hf_{i,\sigma}^\triangle(G)\middle|\sigma \right]\right]
%     \label{eq:thm:l2-loss_II_ast_wo_Lap_ve} \\
%     &\qquad + \frac{n^2(n-1)^2}{36t^2}
%     \E_\sigma\left[\sum_{i \in D} \V_{RR}\left[\hf_{i,\sigma}^\triangle(G)\middle|\sigma \right]\right],
%     \label{eq:thm:l2-loss_II_ast_wo_Lap_ev}
% \end{align}
% The only difference between (\ref{eq:thm:l2-loss_II_ve}) and
% (\ref{eq:thm:l2-loss_II_ast_wo_Lap_ve}) is that the former calculates a sum over
...
% Thus, we can bound the variance in (\ref{eq:thm:l2-loss_II_ast_wo_Lap_ve}) as follows:
% \begin{align*}
%   \V_\sigma\left[\sum_{i = 1, 3, \ldots}^{2t-1}
%     \E_{RR}\left[\hf_{i,\sigma}^\triangle(G)\middle|\sigma \right]\right]
%     &\leq \frac{|D|d_{max}^3}{n-1}.
% \end{align*}
% Similarly, (\ref{eq:thm:l2-loss_II_ev}) calculates a sum over $i=1, 3, \ldots,
% 2t-1$, whereas (\ref{eq:thm:l2-loss_II_ast_wo_Lap_ev}) calculates a sum over $i
...
% we do here.}
% Thus, we can bound the expectation in (\ref{eq:thm:l2-loss_II_ast_wo_Lap_ev}) as follows:
% \[
%   \E_\sigma\left[\sum_{i = 1, 3, \ldots}^{2t-1}
%     \V_{RR}\left[\hf_{i,\sigma}^\triangle(G)\middle|\sigma \right]\right]
...
%   \frac{|D|n^3}{36t^2}d_{max}^3. %\label{eq:thm:l2_II}
% \]
...
\subsection{Proof of Theorem~\ref{thm:l2-loss_IV}}
